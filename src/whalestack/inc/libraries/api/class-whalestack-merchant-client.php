...
class Whalestack_Merchant_Client extends Whalestack_Rest_Client {
     ...
     * @return Whalestack_Rest_Client_Response_Object
     */
    public function get($endpoint = '/', $params = array()) {
        ...
        $this->log("[WhalestackMerchantClient][get] Request: GET $endpoint Params: " . json_encode($params) . " Auth Headers: " . json_encode($authHeaders));
        $this->log("[WhalestackMerchantClient][get] Response: " . json_encode($response));
        return $response;
    ...
    public function post($endpoint = '/', $params = array()) {
        ...
        $this->log("[WhalestackMerchantClient][post] Request: POST $endpoint Params: " . json_encode($params) . " Auth Headers: " . json_encode($authHeaders));
        $this->log("[WhalestackMerchantClient][post] Response: " . json_encode($response));
        return $response;
    }
    ...
    public function delete($endpoint = '/', $params = array()) {
        ...
        $authHeaders = $this->buildAuthHeaders($endpoint, $method, $params);
        $response = $this->sendRequest($endpoint, $method, $params, true, array(), $authHeaders, $this->buildCustomOptions());
        $this->log("[WhalestackMerchantClient][delete] Request: DELETE $endpoint Params: " . json_encode($params) . " Auth Headers: " . json_encode($authHeaders));
        $this->log("[WhalestackMerchantClient][delete] Response: " . json_encode($response));
        return $response;
    ...
    public function put($endpoint = '/', $params = array()) {
        ...
        $authHeaders = $this->buildAuthHeaders($endpoint, $method, $params);
        $response = $this->sendRequest($endpoint, $method, $params, true, array(), $authHeaders, $this->buildCustomOptions());
        $this->log("[WhalestackMerchantClient][put] Request: PUT $endpoint Params: " . json_encode($params) . " Auth Headers: " . json_encode($authHeaders));
        $this->log("[WhalestackMerchantClient][put] Response: " . json_encode($response));
        return $response;
    ...
    private function buildAuthHeaders($path, $method, $params = array()) {
        ...
        $version_data = new \Whalestack\Inc\Core\Init();
    ...
    private function fetchTimestamp() {
        ...
        $timestamp = time();
        $client = new Whalestack_REST_Client('https', 'www.whalestack.com', '/api/v1');
    ...
    private function log($message) {
        ...
        Whalestack_Logging_Service::write($message);
