...
class Payment_Buttons_List_Table extends Libraries\WP_List_Table  {
	...
	public function prepare_items() {
		...
		if (isset($_REQUEST['wp_screen_options']['value'])) {
			$_SESSION['whalestack-buttons-list-value'] = sanitize_text_field($_REQUEST['wp_screen_options']['value']);
		}
		$items = isset($_SESSION['whalestack-buttons-list-value']) ? $_SESSION['whalestack-buttons-list-value'] : 20;
	...
	public static function get_items($per_page = 20, $page_number = 1) {
		...
		$sql = "SELECT * FROM {$wpdb->prefix}whalestack_payment_buttons";
	...
	public static function record_count() {
		...
		$sql = "SELECT COUNT(*) FROM {$wpdb->prefix}whalestack_payment_buttons";
	...
	function column_name($item) {
		...
		$delete_nonce = wp_create_nonce('whalestack_delete_item');

		$title = '<strong>' . esc_html($item['name']) . '</strong>';

		$actions = [
			'edit' => sprintf('<a href="/wp-admin/admin.php?page=whalestack-edit-payment-button&id=%s">' . esc_html(__('Edit', 'whalestack')) .'</a>', absint($item['hashid'])),
			'delete' => sprintf('<a href="?page=%s&action=%s&item=%s&_wpnonce=%s">' . esc_html(__('Delete', 'whalestack')) .'</a>', sanitize_text_field($_REQUEST['page']), 'delete', absint($item['hashid']), $delete_nonce)
		];
    ...
    function column_buttontext($item) {
        $button_text = is_null($item['buttontext']) ? __('Buy Now', 'whalestack') : $item['buttontext'];
	...
	public function handle_table_actions() {
		...
		if ('delete' === $this->current_action()) {
			...
			if (!wp_verify_nonce($nonce, 'whalestack_delete_item')) {
				...
				if ($row->status == 1) {
					$message = esc_html(__('Cannot be deleted when status is active', 'whalestack'));
				...
				$message = esc_html(__('Button successfully deleted', 'whalestack'));
				$page = "whalestack-payment-buttons";
	...
	private static function delete_item($id) {
		...
		$wpdb->delete(
			"{$wpdb->prefix}whalestack_payment_buttons",
			['hashid' => $id],
			['%d']
		);
	}
