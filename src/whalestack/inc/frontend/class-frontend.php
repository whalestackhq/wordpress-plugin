...
class Frontend {
	...
	public function enqueue_styles() {
		wp_enqueue_style($this->plugin_name, plugin_dir_url(__FILE__) . 'css/whalestack.modal.css', array(), $this->version, 'all');
	}
	...
	public function enqueue_scripts() {
        wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__) . 'js/whalestack.modal.min.js', array('jquery'), $this->version, false);

		$params = array ('ajaxurl' => admin_url('admin-ajax.php' ) );
		wp_enqueue_script('whalestack_ajax_handle', plugin_dir_url( __FILE__ ) . 'js/whalestack-frontend-ajax-handler.js', array('jquery'), $this->version, false);
	 ...
	 */
	public function whalestack_render_shortcode_form($params) {
	...
	public function public_form_response_handler() {
        ...
        if (!wp_verify_nonce( $nonce, 'submit_whalestack_checkout_8b%kj@')) {
